...
private open class BufferedAsyncSink( // @formatter:off
    ...
    override suspend fun writeShortLe(value: Short) {
        check(!isClosed.load()) { "AsyncSink is closed" }
        if (buffer.size + Short.SIZE_BYTES >= bufferSize) emit()
        buffer.writeShortLe(value)
    }

    override suspend fun writeIntLe(value: Int) {
        check(!isClosed.load()) { "AsyncSink is closed" }
        if (buffer.size + Int.SIZE_BYTES >= bufferSize) emit()
        buffer.writeIntLe(value)
    }

    override suspend fun writeLongLe(value: Long) {
        check(!isClosed.load()) { "AsyncSink is closed" }
        if (buffer.size + Long.SIZE_BYTES >= bufferSize) emit()
...
private class SynchronizedBufferedAsyncSink(
    ...
    override suspend fun writeShortLe(value: Short) = mutex.withLock {
        super.writeShortLe(value)
    }

    override suspend fun writeIntLe(value: Int) = mutex.withLock {
        super.writeIntLe(value)
    }

    override suspend fun writeLongLe(value: Long) = mutex.withLock {
